...
class TransferEngine {
    ...
    async integrityCheck() {
        ...
        const { sourceSchema, destinationSchema } = await __classPrivateFieldGet(this, _TransferEngine_instances, "m", _TransferEngine_getSchemas).call(this);
        try {
            if (sourceSchema && destinationSchema) {
                __classPrivateFieldGet(this, _TransferEngine_instances, "m", _TransferEngine_assertSchemasMatching).call(this, sourceSchema, destinationSchema);
            }
        }
        catch (error) {
        ...
        const transform = (0, stream_chain_1.chain)([
            ...
            new stream_1.Transform({
                ...
                transform: async (entity, _encoding, callback) => {
                    const { destinationSchema: schemas } = await __classPrivateFieldGet(this, _TransferEngine_instances, "m", _TransferEngine_getSchemas).call(this);
                    if (!schemas) {
...
_TransferEngine_metadata = new WeakMap(), _TransferEngine_schema = new WeakMap(), _TransferEngine_handlers = new WeakMap(), _TransferEngine_currentStream = new WeakMap(), _TransferEngine_instances = new WeakSet(), _TransferEngine_createStageTransformStream = function _TransferEngine_createStageTransformStream(key, options = {}) {
    ...
    const applyTransforms = (transforms = []) => {
        ...
        for (const transform of transforms) {
            if ('filter' in transform) {
                chainTransforms.push((0, stream_2.filter)(transform.filter));
            }
            if ('map' in transform) {
                chainTransforms.push((0, stream_2.map)(transform.map));
            }
        }
    ...
    return {
        sourceSchema: __classPrivateFieldGet(this, _TransferEngine_schema, "f").source,
        destinationSchema: __classPrivateFieldGet(this, _TransferEngine_schema, "f").destination,
    };
};
